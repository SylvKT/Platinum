...
namespace Platinum {
	...
	public:
		int red, green, blue;
		float alpha;
		Color4(int red, int green, int blue, float alpha);
	};
	enum Notification {
		PARENT_ADD
	}
	class Node {
	public:
		vector<weak_ptr<Node>> getChildren();
		vector<weak_ptr<Node>> getParent();
		void addChild(Node* node);
		void notify(Notification notif);
		void on(Notification notif, function<void()> cb);
		~Node();
	private:
		vector<shared_ptr<Node>> children;
		weak_ptr<Node> parent;
		map<Notification, vector<function<void()>>> listeners;
	};
	...
	public:
		...
		void render();
	private:
		shared_ptr<Scene> scene; // The current scene
	};
	enum Key
	{
